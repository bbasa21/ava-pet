...
def install_tictactoe(AvaPetApp, font_name="Roboto"):
    ...
    def _ttt_render_board(self):
        ...
        for index,button in enumerate(self.ttt_buttons):
            ...
            button.disabled=self.ttt_finished or (value=="-" and (self.ttt_selected_pending or self.ttt_turn!="ALI"))
            button.background_color=(0.17,0.055,0.29,1) if value=="-" else (0.36,0.09,0.58,1)
            button.color=(1,1,1,1)
        self.ttt_score.text=f"ALI  {self.ttt_ali_score}     —     {self.ttt_ava_score}  AVA"

    def ttt_select_cell(self,index):
        if self.ttt_finished or self.ttt_turn!="ALI" or self.ttt_selected_pending:return
        if index<0 or index>=9 or self.ttt_board_state[index]!="-":return
    ...
    def ttt_rematch(self,*_):
        if not self.ttt_finished:return
        self.ttt_finished=False
        self.ttt_selected_pending=False
        self.ttt_rematch_button.disabled=True
    ...
    def select_game(self,game_id,_button_event=False):
        ...
        if normalized!=GAME_ID:return original_select_game(self,game_id,_button_event)
    ...
    def show_settings(self,*args):self._ttt_hide();return original_show_settings(self,*args)
    def show_ava_home(self,*args):self._ttt_hide();return original_show_ava_home(self,*args)
    ...
    def start_automatic_scan(self,*args):
        ...
        def watch_for_ava(_dt):
            if self._ttt_auto_connecting:return False
            try:
                if self.ble.has_ava():
