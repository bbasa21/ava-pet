...
class AndroidBLE:
    ...
    def _create_scan_callback(self):
        ...
        class ScanCallback(PythonJavaClass):
            __javainterfaces__ = ["android/bluetooth/BluetoothDevice$LeScanCallback"]
            ...
            def onLeScan(self, device, rssi, scanRecord):
                try:
                    ...
                    outer.log(
                        f"AVA FOUND | name={name} | address={address} | rssi={rssi}"
                    )
    ...
    def connect(self):
        ...
        try:
            ...
            self.log(
                f"GATT CONNECTION REQUESTED | {self.found_name or AVA_NAME} | {self.found_address}"
            )

            # Explicit BLE LE transport. This avoids Android choosing an
            # unsuitable transport on devices that expose AVA through LE.
    ...
    def _handle_java_event(self, event):
        ...
        if kind == "RETRY":
            ...
            self.notifications_enabled = False
            self.log(
                f"GATT RETRY REQUESTED | status={status} | attempt={attempt}"
            )
            return
        ...
        if kind == "STATE":
            ...
            self.log(
                f"GATT STATE CHANGE | status={status} | state={state}"
            )
        ...
        if kind == "SERVICES":
            ...
            try:
                ...
                self.service = self.gatt.getService(
                ...
                self.command_characteristic = self.service.getCharacteristic(
                    UUID.fromString(COMMAND_UUID)
                )
                self.event_characteristic = self.service.getCharacteristic(
                    UUID.fromString(EVENT_UUID)
                )
                self.state_characteristic = self.service.getCharacteristic(
                    UUID.fromString(STATE_UUID)
                )
                self.data_characteristic = self.service.getCharacteristic(
        ...
        if kind == "CHANGED":
            ...
            if (
                uuid.lower() == EVENT_UUID.lower()
                and text.strip().upper() == TIME_REQUEST
            ):
                self.log("[CLOCK] AVA REQUESTED CURRENT TIME.")
                Clock.schedule_once(
                    lambda *_: self.send_current_time_to_ava(),
                    0.05,
                )
        ...
        if kind == "WRITE":
            ...
            if uuid.lower() == COMMAND_UUID.lower():
                self.command_write_busy = False
                if status == GATT_SUCCESS:
                    self.log(f"COMMAND WRITE OK | {uuid}")
                else:
                    self.log(
                        f"COMMAND WRITE FAILED | {uuid} | status={status}"
                    )
                ...
                self.data_write_busy = False
                if status == GATT_SUCCESS:
                    self.log(f"DATA WRITE OK | {uuid}")
                else:
                    self.log(
                        f"DATA WRITE FAILED | {uuid} | status={status}"
                    )
                self._process_data_queue()
            else:
                self.log(
                    f"WRITE EVENT UNKNOWN UUID | {uuid} | status={status}"
                )
        ...
        if kind == "DESCRIPTOR":
            ...
            if status == GATT_SUCCESS:
                ...
                self.ready = False
                self.log(
                    f"CCCD WRITE FAILED | {uuid} | status={status}"
                )
    ...
    def enable_notifications(self):
        ...
        try:
            if not self.gatt.setCharacteristicNotification(
                ...
                self.log(
                    "NOTIFY ERROR: setCharacteristicNotification() FAILED."
                )
                return False
    ...
    def _process_command_queue(self):
        ...
        try:
            ...
            if not self.gatt.writeCharacteristic(
                ...
                self.command_write_busy = False
                self.log(
                    f"COMMAND WRITE REQUEST FAILED | {command} | {name}"
                )
                Clock.schedule_once(
    ...
    def _process_data_queue(self):
        ...
        try:
            ...
            if not self.gatt.writeCharacteristic(
                ...
                self.data_write_busy = False
                self.log(
                    f"DATA WRITE REQUEST FAILED | {data} | {name}"
                )
                Clock.schedule_once(
                    lambda *_: self._process_data_queue(),
                    0.05,
                )
                return
...
class AvaPetApp(App):
    def build(self):
        ...
        # Keep the discovered-name label and CONNECT button as direct children
        # of the page. This removes the extra BoxLayout touch layer and makes
        # the button's hit area deterministic on Android.
    ...
    def on_start(self):
        try:
            ...
            if sdk >= 31:
                ...
                self.add_log(
                    "REQUESTING ANDROID 12+ BLE + LOCATION PERMISSIONS..."
                )
            else:
                permissions = [
                ...
                self.add_log(
                    "REQUESTING ANDROID 10/11 BLE + LOCATION PERMISSIONS..."
                )
    ...
    def connect_ava(self, *_):
        # Immediate UI feedback proves the touch reached this callback before
        # any BLE operation starts.
    ...
    def add_log(self, message):
        ...
        if "AVA FOUND" in upper:
            self.ava_name_label.text = str(
                self.ble.found_name or AVA_NAME
            )
